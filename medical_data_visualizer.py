...
df['overweight'] = np.where(
    (df.weight / (df.height/100)**2) >= 25, True, False)
...
def draw_cat_plot():
    ...
    g = sns.catplot(x='variable', kind='count',
                    data=df_cat, hue='value', col='cardio')
...
def draw_heat_map():
    ...
    sns.heatmap(corr, mask=mask, vmax=.3, center=0, square=True,
                linewidths=.5, cbar_kws={"shrink": .8}, annot=True, fmt=".1f")
